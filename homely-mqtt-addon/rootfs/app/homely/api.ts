...
import type { Home, HomelyLocation } from "../models/home";
import type { HomelySocket } from "../models/homely-socket";
import { appConfig } from "../utils/config";
import { logger } from "../utils/logger";
import { authenticator } from "./auth";

const uri = `https://${appConfig.homely.host}/homely`;
const wsUri = `wss://${appConfig.homely.host}`;
...
export async function listenToSocket(locationId: string) {
	const token = await authenticator.getToken();
	const socket = io(`${wsUri}?locationId=${locationId}&token=Bearer ${token.access_token}`, {
		reconnection: false,
	...
	socket.on("event", async (data: HomelySocket) => {
		logger.trace(data);
		...
		switch (data.type) {
			case "device-state-changed": {
				...
				for (const c of unit.changes) {
					...
					if (!feature) {
						...
						logger.debug(device);
						logger.debug(`Query by ${device.id}_${c.stateName} returned 0 results`);
						return;
					}
			...
			case "alarm-state-changed": {
				...
				if (!gatewayDevice) {
					logger.warn(`Gateway device not found for location ${data.data.locationId}`);
					return;
				}
				...
				if (!feature) {
					logger.warn(`Gateway feature not found for location ${data.data.locationId}`);
					return;
				}
				const stateTopic = feature.state_topic;
			...
			default:
				logger.warn(`Unknown event type ${(data as { type: string }).type}, see payload below:`);
				logger.warn(data);
		}
	});
}
