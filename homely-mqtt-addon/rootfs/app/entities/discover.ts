...
import type { SensorWithName } from "../sensors/model";
import { appConfig } from "../utils/config";
import { getValueByPath } from "../utils/get-value-by-path";
import { logger } from "../utils/logger";

const configTopic = appConfig.mqtt.topicPrefixes?.config ?? "homeassistant";
const stateTopic = appConfig.mqtt.topicPrefixes?.state ?? "homely";
...
export const discover = (
	...
	return matched.flatMap((m) => {
		return m.sensors.map((s) => {
			const device_class =
				s.deviceClass ?? (s.getDeviceClass ? s.getDeviceClass(m.device) : undefined);
